...
const PAYSTACK_PLAN_SETTING_KEYS: Record<string, string> = {
  growth: "paystack_plan_code_realestate_growth",
  pro: "paystack_plan_code_realestate_pro",
};
...
const ensurePaystackPlanCode = async (params: {
  ...
  const createRes = await fetch("https://api.paystack.co/plan", {
    ...
    body: JSON.stringify({
      ...
      interval,
      currency,
      description: "DataPulseFlow Real Estate subscription",
    }),
  });
...
serve(async (req) => {
  ...
  try {
    ...
    const resolvedPlanPrices: Record<string, number> = {
      growth: parsePlanPrice(
        settings?.find((s) => s.setting_key === "realestate_plan_price_growth")?.setting_value,
        PLAN_PRICES_USD.growth,
      ),
      pro: parsePlanPrice(
        settings?.find((s) => s.setting_key === "realestate_plan_price_pro")?.setting_value,
        PLAN_PRICES_USD.pro,
      ),
    };
    ...
    const interval = planKey === "pro" ? "annually" : "monthly";
    const planCodeSetting = PAYSTACK_PLAN_SETTING_KEYS[planKey as keyof typeof PAYSTACK_PLAN_SETTING_KEYS];
    const planDisplayName = planKey === "growth"
      ? `DataPulseFlow Real Estate Monthly (${chargeCurrency})`
      : `DataPulseFlow Real Estate Annual (${chargeCurrency})`;
    ...
    const reference = `DPF_RESUB_${userData.user.id}_${planKey}_${Date.now()}`;
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.error("[create-realestate-paystack-subscription-checkout]", message);
