...
serve(async (req) => {
  ...
  try {
    ...
    const resolvedPlanPrices = {
      growth: parsePlanPrice(getSetting("realestate_plan_price_growth"), PLAN_PRICES.growth),
      pro: parsePlanPrice(getSetting("realestate_plan_price_pro"), PLAN_PRICES.pro),
      enterprise: parsePlanPrice(getSetting("realestate_plan_price_enterprise"), PLAN_PRICES.enterprise),
    };
    ...
    const orderRes = await fetch(`${paypalBase}/v2/checkout/orders`, {
      ...
      body: JSON.stringify({
        ...
        application_context: {
          return_url: `${origin}/real-estate/dashboard?checkout=paypal-success`,
          cancel_url: `${origin}/real-estate/dashboard?checkout=paypal-canceled`,
