...
serve(async (req) => {
  ...
  try {
    ...
    if (await invoiceAlreadyRecorded(supabase, "invoices", keys)) {
      return new Response(
        JSON.stringify({
          status: "COMPLETED",
    ...
    if (productLineMeta === "realestate") {
      throw new Error("Use the Real Estate dashboard to verify this payment.");
    }
